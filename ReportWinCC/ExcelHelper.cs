...
namespace ReportWinCC
{
    class ExcelHelper
    {
        ...
        Worksheet ws;
        public bool border;
        public int round_after_point = 0;
        ...
        public ExcelHelper(string path, int Sheet, bool border, int round)
        {
            this.path = path;
            wb = excel.Workbooks.Open(path);
            ws = excel.Worksheets[Sheet];
            this.border = border;
            this.round_after_point = round;
        }
        ...
        public void WriteCell(int range, int column, string value)
        {
            ...
            if (isNum)
            {
                num = Math.Round(num, round_after_point);
                value = num.ToString();
            }
